#!/usr/bin/env python3
"""
install.py — Bootstrap installer for Synto.
...
def check_ollama() -> None:
    ...
    if shutil.which("ollama"):
        ...
        warn("You can install it later; synto will work once ollama is running.")
...
def verify_install() -> bool:
    """Check that `synto` is on PATH and responds to --version."""
    synto = shutil.which("synto")
    if not synto:
        return False
    result = subprocess.run(["synto", "--version"], capture_output=True, text=True)
...
def fix_windows_path_hint(installer: str) -> None:
    ...
    if installer == "uv":
        ...
        warn(f"Or run directly: {scripts / 'synto.exe'}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Install Synto")
    ...
    if verify_install():
        ok("synto is on PATH and working")
        print()
        rule("━")
        print()
        ...
        print()
        print(f"    {bold('synto setup')}")
        print()
        info("This configures your Ollama URL, models, and default vault.")
        print()
    else:
        warn("synto was installed but not found on PATH.")

...
from .markdown_math import mask_markdown_regions, restore_markdown_regions
...
def extract_wikilinks(content: str) -> list[str]:
    ...
    """
    masked, _ = mask_markdown_regions(content, mask_wikilinks=False, mask_embeds=False)
...
def _mask_code_blocks(content: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    """
    return mask_markdown_regions(content, mask_wikilinks=False, mask_embeds=True)
...
def list_wiki_articles(wiki_dir: Path) -> list[tuple[str, Path]]:
    """Return [(title, path)] for published concept articles only."""
    articles = []
    for md in wiki_dir.rglob("*.md"):
        rel_path = md.relative_to(wiki_dir)
        if not is_concept_article_path(str(rel_path)):
            continue
        try:
...
def is_concept_article_path(relative_path: str) -> bool:
    parts = Path(relative_path).parts
    if ".drafts" in parts:
        return False
    lowered_parts = {part.casefold() for part in parts}
    if {"sources", "queries", "synthesis"} & lowered_parts:
        return False

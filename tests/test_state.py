...
@pytest.fixture
def db(tmp_path):
    return StateDB(tmp_path / ".synto" / "state.db")
...
def test_insert_synthesis_atomic_is_standalone_atomic(db):
    first = WikiArticleRecord(
        path="wiki/synthesis/topic.md",
        title="Topic",
        sources=[],
    ...
    db.insert_synthesis_atomic(first)
    with pytest.raises(DuplicateSynthesisQuestionHashError):
        db.insert_synthesis_atomic(second)

    articles = [a for a in db.list_articles() if a.kind == "synthesis"]
    assert [article.path for article in articles] == ["wiki/synthesis/topic.md"]

...
from synto.config import Config, PipelineConfig, default_wiki_toml
...
def test_from_vault_rejects_legacy_telemetry_section(tmp_path):
    _write_wiki_toml(
        tmp_path,
        """
[models]
...
""",
    ...
    with pytest.raises(ValueError, match=r"Legacy \[telemetry\] config found"):
...
def _write_wiki_toml(path: Path, body: str) -> None:
    ...
    (path / "synto.toml").write_text(body)

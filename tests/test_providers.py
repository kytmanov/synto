...
from synto.openai_compat_client import LLMBadRequestError, LLMError, OpenAICompatClient
from synto.providers import (
    get_provider,
    list_all_providers,
...
def test_generate_temperature_set_when_provided():
    client = _make_client()
    mock_resp = MagicMock()
    mock_resp.status_code = 200
    mock_resp.json.return_value = {"choices": [{"message": {"content": "x"}}]}
    ...
    with patch.object(client._client, "post", side_effect=fake_post):
        client.generate("p", model="m", temperature=0)
...
def test_generate_429_backoff_retries(monkeypatch):
    ...
    slept: list[float] = []
    monkeypatch.setattr(
        "synto.openai_compat_client.time.sleep",
...
def test_build_client_openai_compat(tmp_path):
    from synto.client_factory import build_client
    from synto.config import Config, ProviderConfig
...
def test_build_client_resolves_generic_synto_api_key(tmp_path):
    from synto.client_factory import build_client
    from synto.config import Config, ProviderConfig
    ...
    with patch.dict("os.environ", {"SYNTO_API_KEY": "generic-key"}, clear=False):
        client = build_client(config)
...
def test_effective_provider_legacy_ollama_section(tmp_path):
    ...
    from synto.config import Config, OllamaConfig
...
def test_effective_provider_new_section_takes_priority(tmp_path):
    ...
    from synto.config import Config, OllamaConfig, ProviderConfig
...
# ── azure_api_version in synto.toml ────────────────────────────────────────────
...
def test_setup_wizard_cloud_provider_empty_api_key(tmp_path, cfg_dir):
    ...
    from synto.providers import list_all_providers

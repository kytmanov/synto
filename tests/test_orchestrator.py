...
from synto.pipeline.orchestrator import (
    FailureReason,
    FailureRecord,
...
def test_run_compile_ollama_error_classified_as_transient(config, db):
    ...
    from synto.ollama_client import OllamaError
...
def test_orchestrator_selective_recompile_with_absolute_paths(config, db):
    ...
    import synto.pipeline.ingest as ingest_mod
...
def test_orchestrator_fix_creates_stubs(config, db):
    ...
    import synto.pipeline.lint as lint_mod
    import synto.pipeline.maintain as maintain_mod
    from synto.models import LintIssue, LintResult

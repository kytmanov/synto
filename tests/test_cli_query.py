...
def test_query_cli_returns_nonzero_on_synthesis_chain_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("synto.pipeline.query.run_query", _raise)
...
def test_query_cli_only_prompts_when_duplicate_exists(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("synto.pipeline.query.run_query", fake_run_query)
...
def test_query_cli_no_index_message_uses_synto(tmp_path, monkeypatch):
    vault = _make_vault(tmp_path)
    config = Config(vault=vault)
    db = StateDB(config.state_db_path)
    ...
    result = CliRunner().invoke(cli, ["query", "--vault", str(vault), "What now?"])

    assert result.exit_code == 0
    assert "synto ingest" in result.output

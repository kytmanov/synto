...
from synto.compare.runner import run_compare
from synto.config import Config
from synto.pipeline.orchestrator import FailureReason, FailureRecord, PipelineReport
...
def patched_pipeline(monkeypatch):
    ...
    monkeypatch.setattr("synto.client_factory.build_client", lambda cfg: fake_client)

    fake_db = MagicMock()
    fake_db.close = MagicMock()
    monkeypatch.setattr("synto.state.StateDB", lambda _path: fake_db)
    ...
    monkeypatch.setattr("synto.pipeline.orchestrator.PipelineOrchestrator.run", fake_run)

    from synto.pipeline.query import QueryRunResult

    def fake_query(**_kw):
        return QueryRunResult(answer="Answer.", selected_pages=["page1"])

    monkeypatch.setattr("synto.pipeline.query.run_query", fake_query)
    ...
    monkeypatch.setattr("synto.pipeline.lint.run_lint", fake_lint)
...
def test_run_compare_writes_artifacts(tmp_path, patched_pipeline):
    ...
    report = run_compare(current, challenger, vault / ".synto" / "compare")
    root = vault / ".synto" / "compare" / report.run_id
...
def test_run_compare_keeps_active_vault_unchanged(tmp_path, patched_pipeline):
    ...
    before = {
        ...
        if p.is_file() and ".synto/compare" not in str(p)
    }
    current = Config.from_vault(vault)
    challenger = Config.from_vault(vault, models={"heavy": "new-heavy"})
    run_compare(current, challenger, vault / ".synto" / "compare")
    after = {
        p.relative_to(vault): p.read_bytes()
        for p in sorted(vault.rglob("*"))
        if p.is_file() and ".synto/compare" not in str(p)
...
def test_run_compare_queries_saved_only_in_artifacts(tmp_path, patched_pipeline):
    ...
    current = Config.from_vault(vault)
    challenger = Config.from_vault(vault, models={"heavy": "new-heavy"})
    report = run_compare(current, challenger, vault / ".synto" / "compare", queries_path=queries)
    root = vault / ".synto" / "compare" / report.run_id
...
def test_run_compare_rejects_symlinked_queries_before_loading(tmp_path, patched_pipeline):
    ...
    with pytest.raises(ValueError, match="must not be a symlink"):
        run_compare(current, challenger, vault / ".synto" / "compare", queries_path=link)
...
def test_run_compare_copies_vault_schema_into_preview(tmp_path, patched_pipeline):
    ...
    root = vault / ".synto" / "compare" / report.run_id / "vaults"
...
def test_run_compare_sample_n_limits_notes(tmp_path, patched_pipeline):
    ...
    current = Config.from_vault(vault)
    challenger = Config.from_vault(vault, models={"heavy": "new-heavy"})
    report = run_compare(current, challenger, vault / ".synto" / "compare", sample_n=2)
    root = vault / ".synto" / "compare" / report.run_id
...
def test_run_compare_rejects_negative_sample_n(tmp_path, patched_pipeline):
    ...
    with pytest.raises(ValueError, match="sample_n must be at least 1"):
        run_compare(current, challenger, vault / ".synto" / "compare", sample_n=-1)
...
def test_run_compare_serializes_failure_error_messages(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "synto.pipeline.query.run_query",
